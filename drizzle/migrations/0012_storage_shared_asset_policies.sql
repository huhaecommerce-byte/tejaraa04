DROP POLICY IF EXISTS "shared_assets_read" ON storage.objects;
...
DROP POLICY IF EXISTS "shared_assets_write" ON storage.objects;
...
DROP POLICY IF EXISTS "shared_assets_update" ON storage.objects;
CREATE POLICY "shared_assets_update" ON storage.objects FOR UPDATE TO authenticated
USING (bucket_id IN ('avatars','platform-icons','brand-assets'));

DROP POLICY IF EXISTS "shared_assets_delete" ON storage.objects;
...
DROP POLICY IF EXISTS "private_files_own" ON storage.objects;
