DROP FUNCTION IF EXISTS public.hunt_catalog_products(text[], text, numeric, numeric, text, boolean, integer, integer, boolean);
...
BEGIN
  ...
  FOREACH v_term IN ARRAY v_terms LOOP
    v_where := v_where || format(' AND p.search_text LIKE %L', '%' || v_term || '%');
    v_score := v_score || format(
      ' + (CASE WHEN lower(p.name) LIKE %1$L OR lower(COALESCE(p.name_ar,'''')) LIKE %1$L THEN 8'
      || ' WHEN lower(p.name) LIKE %2$L OR lower(COALESCE(p.name_ar,'''')) LIKE %2$L THEN 5'
      || ' WHEN lower(p.sku) LIKE %2$L THEN 4 ELSE 2 END)',
      v_term || '%', '%' || v_term || '%');
  END LOOP;
  ...
  IF _category IS NOT NULL AND length(trim(_category)) > 1 THEN
    v_where := v_where || format(' AND p.search_text LIKE %L', '%' || lower(trim(_category)) || '%');
  END IF;
  ...
  v_sql := format($q$
    ...
    SELECT s.id, s.sku, s.name, s.name_ar, s.top_category, s.sub_category,
      ...
      s.dropship_price_usd, s.moq::numeric, s.weight_kg, s.estimated_delivery,
...
GRANT EXECUTE ON FUNCTION public.hunt_catalog_products(text[], text, numeric, numeric, text, boolean, integer, integer, boolean) TO anon, authenticated, service_role;