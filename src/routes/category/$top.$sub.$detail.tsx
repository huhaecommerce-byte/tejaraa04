...
export const Route = createFileRoute('/category/$top/$sub/$detail')({
  validateSearch: (search) => ({ page: readPage(search) }),
  loaderDeps: ({ search }) => ({ page: search.page }),
  loader: async ({ params, deps }) => {
    const data = await getCategoryPage({
      data: { top: params.top, sub: params.sub, detail: params.detail, page: deps.page - 1 },
    });
  ...
  head: ({ loaderData }) => {
    ...
    const path = categoryPath(loaderData.top, loaderData.sub, loaderData.detail);
