...
export const Route = createFileRoute('/category/$top')({
  validateSearch: (search) => ({ page: readPage(search) }),
  loaderDeps: ({ search }) => ({ page: search.page }),
  loader: async ({ params, deps }) => {
    const data = await getCategoryPage({ data: { top: params.top, page: deps.page - 1 } });
  ...
  head: ({ loaderData }) => {
    ...
    const url = `https://tejaraa.com${categoryPath(loaderData.top)}`;
    return {
      meta: [
        { title },
      ...
      links: [
        ...
        { rel: 'alternate', hrefLang: 'ar-sa', href: `https://tejaraa.com/ar${categoryPath(loaderData.top)}` },
        { rel: 'alternate', hrefLang: 'x-default', href: url },
      ],
      scripts: [
        {
          type: 'application/ld+json',
          children: JSON.stringify(categoryJsonLd(loaderData)),
        },
      ],
    };
  },
