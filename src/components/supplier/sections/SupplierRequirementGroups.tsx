    ...
    <section className="bg-retail-page py-11 lg:py-14">
      <SupplierContainer>
        <SupplierSectionHeading
          eyebrow="Requirement groups"
          title="What we ask for, and when"
          description="Asked at application, asked during review, and what keeps supply running afterwards."
        />
            ...
            <article key={group.title} className="rounded-2xl border border-retail-border bg-white p-5">
              <div className="flex items-start gap-4">
                ...
                <div className="min-w-0">
                  <h3 className="text-base font-bold text-retail-dark-green">{group.title}</h3>
                  <p className="mt-1.5 text-sm leading-6 text-retail-muted">{group.intro}</p>
                  ...
                  {group.note ? (
                    <p className="mt-3 rounded-lg bg-retail-page p-3 text-xs leading-5 text-retail-muted">{group.note}</p>
                  ) : null}
                </div>
              </div>
            </article>
