  ...
  return (
    <section className="min-w-0 py-1">
      ...
      <div className="mt-3 flex flex-wrap items-center gap-x-4 gap-y-2 text-xs text-retail-muted">
        {hasRating && (
          <a href="#reviews" className="inline-flex items-center gap-1 font-semibold text-retail-text hover:text-retail-green">
            ...
            <span className="font-normal text-retail-muted">({product.review_count} reviews)</span>
          </a>
        )}
        {product.sku && <span>SKU: <span className="font-semibold text-retail-text">{product.sku}</span></span>}
      </div>
      ...
      <div className="my-5 border-y border-retail-border py-4">
        ...
        <p className="mt-1 text-xs text-retail-muted">Price excludes VAT. Shipping is calculated at checkout.</p>
      </div>
      ...
      <div className="space-y-3">
        <div className="flex items-start gap-3">
          ...
          <div><p className={outOfStock ? 'font-bold text-retail-sale' : lowStock ? 'font-bold text-retail-gold-foreground' : 'font-bold text-retail-green'}>{availability}</p><p className="text-xs text-retail-muted">{product.source === 'local' ? 'Ships from stock in Saudi Arabia.' : 'Imported through standard delivery.'}</p></div>
        </div>
        {product.estimated_delivery && (
          <div className="flex items-start gap-3">
            ...
            <div><p className="font-semibold text-retail-text">Estimated delivery</p><p className="text-xs text-retail-muted">{product.estimated_delivery}</p></div>
          </div>
        )}
      </div>
    </section>
