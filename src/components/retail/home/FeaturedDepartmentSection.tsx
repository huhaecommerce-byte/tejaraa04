  ...
  return (
    <section className="grid overflow-hidden rounded-lg border border-retail-border bg-retail-card lg:grid-cols-[260px_minmax(0,1fr)]">
      <Link to={href} className="group relative min-h-[180px] overflow-hidden lg:min-h-full">
        ...
        <div className="absolute inset-x-0 bottom-0 z-10 p-5 text-primary-foreground"><p className="text-[10px] font-bold uppercase text-retail-gold">{t.eyebrow}</p><h2 className="mt-1 font-display text-2xl font-bold">{name}</h2><span className="mt-3 inline-flex items-center gap-1 text-xs font-bold">{t.action} <ArrowRight className="h-3.5 w-3.5 rtl:rotate-180" /></span></div>
      </Link>
