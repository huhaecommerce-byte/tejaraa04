    ...
    <section className="border-y border-retail-border bg-white py-11 lg:py-14">
      <SellerContainer>
        <SellerSectionHeading
          eyebrow={t('selling.audience.eyebrow')}
          title={t('selling.audience.title')}
          description={t('selling.audience.desc')}
          align="center"
        />
        <div className="mt-7 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          {sellerAudiences.map((audience) => (
            <div key={audience.id} className="flex h-full flex-col rounded-xl border border-retail-border bg-retail-page/70 p-5">
              ...
              <h3 className="mt-4 text-base font-bold text-retail-dark-green">{t(audience.titleKey)}</h3>
              <p className="mt-2 text-sm leading-6 text-retail-muted">{t(audience.textKey)}</p>
              <Link
                to={audience.to}
