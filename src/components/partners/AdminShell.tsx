...
const navSections: NavSection[] = [
  {
    title: "Dashboard",
    items: [
      { label: "Overview", to: "/partners/admin", icon: LayoutDashboard },
    ],
  },
  {
    title: "Wholesalers",
    items: [
      { label: "Pending Applications", to: "/partners/admin/applications", icon: BadgeCheck },
      { label: "Wholesalers", to: "/partners/admin/suppliers", icon: Users },
    ],
  },
  {
    title: "Products",
    items: [
      { label: "Product Approvals", to: "/partners/admin/products", icon: ClipboardCheck },
      { label: "Live Products", to: "/partners/admin/live-products", icon: PackageCheck },
    ],
  },
  {
    title: "Sales & Money",
    items: [
      { label: "Orders & Performance", to: "/partners/admin/orders", icon: ShoppingCart },
      { label: "Payout Details", to: "/partners/admin/payouts", icon: Wallet },
    ],
  },
  {
    title: "Platform",
    items: [
      { label: "Market Flags", to: "/partners/admin/markets", icon: Flag },
      { label: "Project Connections", to: "/partners/admin/connections", icon: Plug },
      { label: "Supplier Integrations", to: "/partners/admin/supplier-integrations", icon: Boxes },
      { label: "Team & Roles", to: "/partners/admin/team", icon: ShieldCheck },
    ],
  },
];

function visibleSections(roles: AppRole[]): NavSection[] {
  return navSections
...
function NavList({ roles, onNavigate }: { roles: AppRole[]; onNavigate?: () => void }) {
  return (
    <nav className="space-y-3">
      {visibleSections(roles).map((section) => (
...
const mobileNav = [
  { label: "Overview", to: "/partners/admin", icon: LayoutDashboard },
  { label: "Applications", to: "/partners/admin/applications", icon: BadgeCheck },
  { label: "Wholesalers", to: "/partners/admin/suppliers", icon: Users },
  { label: "Products", to: "/partners/admin/products", icon: ClipboardCheck },
] as const;
    ...
    <div className="supplier-theme min-h-screen overflow-x-clip bg-secondary/40 text-foreground">
      <div className="mx-auto flex w-full max-w-[1720px] gap-4 lg:px-6 lg:py-5">
        <aside className="sticky top-5 hidden h-[calc(100vh-2.5rem)] w-56 shrink-0 flex-col rounded-2xl border border-border bg-card p-3 shadow-card lg:flex">
          ...
          >
            <Store className="h-4 w-4" /> View as supplier
          </Link>
          <button
            type="button"
            onClick={signOut}
          ...
          <div className="fixed inset-0 z-50 lg:hidden">
            ...
            <div className="absolute inset-y-0 left-0 flex w-[min(21rem,88vw)] flex-col overflow-y-auto bg-card px-4 pb-[calc(1rem+env(safe-area-inset-bottom))] pt-[calc(1rem+env(safe-area-inset-top))] shadow-nav">
              ...
              <div className="mt-5 border-t pt-3">
                ...
                <button type="button" onClick={signOut} className="mt-1 flex w-full items-center gap-2.5 rounded-xl px-2.5 py-2 text-xs font-semibold text-destructive hover:bg-destructive/10"><LogOut className="h-4 w-4" /> Sign out</button>
              </div>
            </div>
          </div>
        )}
        ...
        <main className="min-w-0 flex-1 pb-[calc(5.5rem+env(safe-area-inset-bottom))] lg:pb-0">
          <header className="sticky top-0 z-30 grid grid-cols-[auto_minmax(0,1fr)] items-center gap-3 border-b border-border bg-card/95 px-3 pb-2.5 pt-[calc(.625rem+env(safe-area-inset-top))] backdrop-blur-xl lg:static lg:flex lg:rounded-2xl lg:border lg:px-3 lg:py-2.5 lg:shadow-card">
            ...
            <div className="min-w-0"><p className="hidden text-[11px] font-bold uppercase tracking-[0.14em] text-primary lg:block">Admin Console</p><p className="truncate text-sm font-extrabold lg:hidden">{title}</p><p className="truncate text-[10px] text-muted-foreground lg:hidden">Admin Console</p></div>
          </header>
          ...
          {readOnly && (
            <div className="mt-3 rounded-2xl border border-border bg-card px-3 py-2 text-xs font-semibold text-muted-foreground lg:mx-0">
              View-only access: you can read everything on this page, but changes are turned off for your role.
            </div>
          )}
      ...
      <nav className="fixed inset-x-0 bottom-0 z-40 grid grid-cols-5 border-t border-border bg-card/95 px-1 pb-[env(safe-area-inset-bottom)] shadow-nav backdrop-blur-xl lg:hidden" aria-label="Admin navigation">
        ...
        <button type="button" onClick={() => setOpen(true)} className="flex min-h-16 flex-col items-center justify-center gap-1 text-[10px] font-bold text-muted-foreground" aria-label="More admin pages"><MoreHorizontal className="h-5 w-5" /><span>More</span></button>
      </nav>
    </div>
  );
}
