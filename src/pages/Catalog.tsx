...
export default function Catalog() {
  ...
  const chips = [...selected.map((item) => ({ key: `cat:${item.key}`, label: item.label })), ...(query ? [{ key: 'q', label: `“${query}”` }] : []), ...(source !== 'all' ? [{ key: 'source', label: source === 'local' ? 'Local / faster delivery' : 'Standard delivery' }] : [])];
  const clearAll = () => setParams(new URLSearchParams());
  const removeChip = (key: string) => updateParams({ [key.startsWith('cat:') ? 'cat' : key]: null, page: null });
  const resultLabel = totalCount === null ? `${products.length.toLocaleString()} products` : `${totalCount.toLocaleString()}${totalCount >= 1000 ? '+' : ''} products`;
  const totalPages = totalCount === null ? 1 : Math.max(1, Math.ceil(totalCount / PAGE_SIZE));

  const filters = <><FilterSection value="delivery" title="Delivery"><div className="grid gap-1">{([['all','All delivery'],['local','Local / faster delivery'],['global','Standard delivery']] as const).map(([value,label]) => <Button key={value} type="button" variant="ghost" onClick={() => updateParams({ source: value === 'all' ? null : value, page: null })} className={`justify-start ${source === value ? 'bg-retail-light-green text-retail-green' : ''}`}>{label}</Button>)}</div></FilterSection><FilterSection value="category" title="Category"><CategorySidebar aggregatedCounts={aggregatedCounts.length ? aggregatedCounts : undefined} loading={categoryLoading} selection={selection} onChange={(next) => updateParams({ cat: [...next.keys][0] ?? null, page: null })} className="border-0 bg-transparent shadow-none" /></FilterSection></>;

  return <RetailPublicShell><JsonLd data={{ '@context': 'https://schema.org', '@type': 'CollectionPage', name: 'Tejaraa Shop Catalogue', description: 'Browse products available on Tejaraa across Saudi Arabia.', url: 'https://tejaraa.com/catalog', isPartOf: { '@type': 'WebSite', name: 'Tejaraa', url: 'https://tejaraa.com' } }} /><main><RetailContainer className="space-y-4 py-3 sm:py-4"><RetailListingHeader breadcrumbs={[{ label: 'Home', to: '/' }, { label: 'All products' }]} title="Shop all products" description="Discover everyday products available across Saudi Arabia." countLabel={!loading ? resultLabel : undefined} /><div className="grid items-start gap-4 lg:grid-cols-[250px_minmax(0,1fr)]"><RetailFilterSidebar hasFilters={chips.length > 0} onClear={clearAll}>{filters}</RetailFilterSidebar><div className="min-w-0 space-y-3"><ListingToolbar resultLabel={loading ? 'Loading products…' : resultLabel} mobileFilters={<MobileFilterSheet count={chips.length} resultLabel={resultLabel} hasFilters={chips.length > 0} onClear={clearAll}>{filters}</MobileFilterSheet>} /><ActiveFilterChips chips={chips} onRemove={removeChip} onClear={clearAll} />{error ? <section className="rounded-lg border border-retail-border bg-retail-card"><RetailErrorState title="We couldn't load these products" description="Please try again to continue shopping." onRetry={() => void runFetch()} /></section> : <RetailProductGrid products={products as (ProductData & { slug?: string | null })[]} loading={loading} emptyTitle="No products found" emptyDescription="Try another category or browse all departments." />}<RetailPagination currentPage={page + 1} totalPages={totalPages} onPageChange={(next) => { updateParams({ page: next > 1 ? String(next) : null }); document.getElementById('listing-results')?.scrollIntoView({ behavior: 'smooth' }); }} /></div></div></RetailContainer></main></RetailPublicShell>;
}