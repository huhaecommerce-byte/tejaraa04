...
import { agencyAdminNavigation } from '@/config/navigation';
import { useStaffPermissions } from '@/hooks/useStaffPermissions';
import { safeSetItem } from '@/lib/safeStorage';

/**
 * Standalone admin portal for the Agencies & VAs programme.
 * Same chrome as the main admin panel but with its own, agency-only sidebar.
 */
const AgencyAdminLayout = () => {
  ...
  const [collapsed, setCollapsed] = useState(() => {
    ...
    return localStorage.getItem('agency-admin-panel-collapsed') === '1';
  });
  ...
  const toggle = () => {
    setCollapsed((c) => {
      const next = !c;
      safeSetItem('agency-admin-panel-collapsed', next ? '1' : '0');
      return next;
    });
  };
  ...
  const allowed = user.role === 'admin' || staffPerms.has('agencies');

  return (
    <div className="min-h-screen w-full bg-[hsl(var(--muted)/0.5)]">
      <CommandDeck
        sections={agencyAdminNavigation}
        variant="admin"
        collapsed={collapsed}
        onToggle={toggle}
        mobileOpen={mobileOpen}
        onMobileClose={() => setMobileOpen(false)}
        staffBadge="Agency programme"
      />
      <button
          ...
          <div className="max-w-[1400px] mx-auto space-y-6">
            {allowed ? (
              ...
              <div className="max-w-md mx-auto mt-12 rounded-2xl border bg-card p-8 text-center shadow-sm">
                ...
                <h2 className="text-lg font-bold mb-2">No access to the agency programme</h2>
                <p className="text-sm text-muted-foreground mb-5">
                  Ask an administrator to grant you the Agencies &amp; VAs module.
                </p>
                <Link to="/admin" className="inline-flex items-center justify-center h-10 px-5 rounded-full bg-primary text-primary-foreground text-sm font-semibold hover:bg-primary/90 transition">
                  Back to admin
                </Link>
              </div>
            )}
