...
CREATE INDEX IF NOT EXISTS idx_products_created_at_top_category ON public.products (created_at DESC, top_category);

CREATE OR REPLACE FUNCTION public.product_digest_stats(_since timestamp with time zone, _limit integer DEFAULT 6)
 RETURNS jsonb
 LANGUAGE plpgsql
...
BEGIN
  ...
  -- Fast planner estimate instead of a full table count.
  SELECT greatest(coalesce(c.reltuples, 0), 0)::bigint INTO _total_catalog
  FROM pg_class c JOIN pg_namespace n ON n.oid = c.relnamespace
  WHERE n.nspname = 'public' AND c.relname = 'products';
  IF coalesce(_total_catalog, 0) = 0 THEN
