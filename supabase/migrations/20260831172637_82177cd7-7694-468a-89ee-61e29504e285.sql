...
DECLARE
  ...
  v_term text;
  v_where text := 'TRUE';
  v_term_clauses text[] := ARRAY[]::text[];
  v_score text := '0';
  v_sql text;
BEGIN
  ...
  FOREACH v_term IN ARRAY v_terms LOOP
    v_term_clauses := v_term_clauses || format('p.search_text LIKE %L', '%' || v_term || '%');
    v_score := v_score || format(
      ...
      || ' WHEN lower(p.sku) LIKE %2$L THEN 4'
      || ' WHEN p.search_text LIKE %2$L THEN 2 ELSE 0 END)',
      v_term || '%', '%' || v_term || '%');
  END LOOP;

  IF array_length(v_term_clauses, 1) > 0 THEN
    IF _relaxed THEN
      v_where := v_where || ' AND (' || array_to_string(v_term_clauses, ' OR ') || ')';
    ELSE
      v_where := v_where || ' AND (' || array_to_string(v_term_clauses, ' AND ') || ')';
    END IF;
  END IF;
  ...
  END IF;
  IF NOT _relaxed AND _category IS NOT NULL AND length(trim(_category)) > 1 THEN
